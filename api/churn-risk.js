// /api/churn-risk - Multi-signal churn scoring for GovChain
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const sub of subs || []) {
      ...
      // Signal 1: No pipeline activity in 7 days
      const { count: recentPipeline } = await supabase
        .from('pipeline').select('id', { count: 'exact', head: true })
        .eq('user_id', sub.user_id).gte('updated_at', sevenDaysAgo);
      if ((recentPipeline ?? 0) === 0) { score += 30; signals.push('No pipeline activity in 7 days'); }

      // Signal 2: No proposals in 30 days
      const { count: recentProposals } = await supabase
        .from('proposal').select('id', { count: 'exact', head: true })
        .eq('user_id', sub.user_id).gte('created_at', thirtyDaysAgo);
      if ((recentProposals ?? 0) === 0) { score += 20; signals.push('No proposals in 30 days'); }

export default {
  async fetch(request, env) {
    ...
    const metric = url.searchParams.get('metric') || 'contracts';
    ...
    const orderCol = metric === 'milestones' ? 'milestone_count'
      ...
      : metric === 'grants' ? 'total_grants'
      : 'total_contracts';

    const res = await fetch(
      `${env.SUPABASE_URL}/rest/v1/user_stats?select=user_id,total_contracts,total_grants,streak_days,milestone_count&order=${orderCol}.desc&limit=${limit}`,
    ...
    const leaderboard = (rows || []).map((row, i) => ({
      ...
      score: metric === 'milestones' ? row.milestone_count
           ...
           : metric === 'grants' ? row.total_grants
           : row.total_contracts,

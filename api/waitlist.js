...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    const { email, name, agency, role, interest } = req.body;
    ...
    const { error } = await supabase.from('waitlist').insert({
      email, name, agency, role, interest,
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: "You're on the GovChain waitlist!",
      html: `<h2>Welcome, ${name || 'friend'}!</h2><p>You're #${count} on the GovChain waitlist. We're rolling out access to federal contractors and small businesses prioritized by registration date.</p><p>Make sure your SAM.gov registration is current - we'll check it when onboarding you.</p>`,
    }).catch(() => {});

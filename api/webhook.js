...
// Handles Stripe webhook events for GovChain subscription activation
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        const tier = session.metadata?.tier || 'municipal';

        await supabaseUpsert('govchain_subscriptions', {
      ...
      case 'customer.subscription.updated': {
        ...
        await supabaseUpdate(
          'govchain_subscriptions',
          { stripe_subscription_id: sub.id },
          { status: sub.status, tier: sub.items?.data?.[0]?.price?.nickname || 'municipal', updated_at: new Date().toISOString() },
          supabaseUrl, supabaseKey
        );
        break;
      }

  ...
  if (req.method === 'GET') {
    try {
      ...
      const { data: earnings } = await supabase
        ...
        .select('amount, status, created_at, referral_plan')
      ...
      return res.status(200).json({
        ...
        affiliate: {
          code: affiliate.code,
          affiliate_url: `https://govchain.us?ref=${affiliate.code}`,
  ...
  if (req.method === 'POST') {
    try {
      ...
      return res.status(201).json({
        success: true,
        affiliate_code: code,
        affiliate_url: `https://govchain.us?ref=${code}`,
        commission_rate: COMMISSION_RATE,
        message: 'Welcome to the GovChain Partner Program! Earn 15% recurring commission on every contractor you refer.',
      });
    } catch (err) {
      return res.status(500).json({ error: err.message });
    }
